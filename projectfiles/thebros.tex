...
\begin{equation}
	...
	\end{array}\right) = 
	\left(\begin{array}{c}
		L_{TR} \widehat B_T + (L_{BL}^T)^T \widehat B_B  \\ \whline
		\widehat B_B
	\end{array}\right).
\label{eq:inv2}
	\end{equation} \\
...
The unblocked algorithm for loop invariant \eqref{eq:inv3} is showing in Figure~\ref{fig:unb_inv1}. While the blocked algorithm for loop invariant \eqref{eq:inv3} is showing in Figure~\ref{fig:blk_inv3}.


\resetsteps

\input{derivations/Trmm_lltn_unb_var1}
...
\input{derivations/Syrk_ln_unb_var1}
...
\input{derivations/Syrk_ln_blk_var1}
